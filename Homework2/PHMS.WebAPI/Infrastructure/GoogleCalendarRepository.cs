...
namespace Infrastructure
{
    public class GoogleCalendarRepository : IGoogleCalendarRepository
    {
        ...
        private const string CalendarId = "predictsmarthealth@gmail.com"; // ID-ul real al calendarului
        ...
        public async Task<bool> EventExistsAsync(DateTime startTime, DateTime endTime, string description)
        {
            ...
            request.TimeMin = startTime.AddMinutes(-5);
            request.TimeMax = endTime.AddMinutes(5);
            ...
            return events.Any(e =>
                ...
                e.Start.DateTime.Value == startTime &&
                e.End.DateTime.Value == endTime
            );
        }
        ...
        public async Task DeleteEventAsync(DateTime startTime, DateTime endTime, string description)
        {
            ...
            var request = _calendarService.Events.List(CalendarId);
            request.TimeMin = startTime.AddMinutes(-5); // Extindem intervalul pentru siguranță
            request.TimeMax = endTime.AddMinutes(5);
            request.ShowDeleted = false;
            request.SingleEvents = true;
            ...
            foreach (var ev in events.Items)
            {
                ...
                if (ev.Description != null && ev.Description.Contains(description, StringComparison.OrdinalIgnoreCase) &&
                    ...
                    ev.Start.DateTime.Value >= startTime.AddMinutes(-5) &&
                    ev.End.DateTime.Value <= endTime.AddMinutes(5))
                {
